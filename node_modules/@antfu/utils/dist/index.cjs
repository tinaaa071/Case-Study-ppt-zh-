...
class PInstance extends Promise {
  ...
  get promise() {
    ...
    if ((_a = this.options) == null ? undefined : _a.concurrency) {
...
function sleep(ms, callback) {
  return new Promise(
    (resolve) => setTimeout(async () => {
      await (callback == null ? undefined : callback());
      resolve();
    }, ms)
  );
}
...
function unindent(str) {
  ...
  const commonIndent = lines.reduce((min, line, idx) => {
    ...
    if (whitespaceLines[idx])
      return min;
    const indent = (_a = line.match(/^\s*/)) == null ? undefined : _a[0].length;
...
function debounce (delay, callback, options) {
  var _ref = options || {},
      ...
      atBegin = _ref$atBegin === undefined ? false : _ref$atBegin;
